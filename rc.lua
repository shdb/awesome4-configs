...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the wibox
    wdir = "north"
    if s.index == 1 and screen:count() > 1 then
        s.mywibox = awful.wibar({position = "left", screen = s, width = beautiful.menu_height})
        wdir = "east"
...
globalkeys = gears.table.join(
    ...
    awful.key({modkey, shift}, ",", function() xrandr.xrandr()                  end,
              {description = "arrange multiple screens", group = "screen"}),
    ...
    awful.key({modkey, ctrl}, "Left", function () awful.tag.incncol( 1, nil, true)    end,
              {description = "increase the number of columns", group = "layout"}),
    awful.key({modkey, ctrl}, "Right", function () awful.tag.incncol(-1, nil, true)    end,
...
--function border_adjust(c, notfocus)
--    if c.maximized or #awful.screen.focused().tiled_clients == 1 and not c.floating == true then
--        c.border_width = 0
